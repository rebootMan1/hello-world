...
def _score_sentences(sentences,important_words):
    ...
    for s in [nltk.tokenize.word_tokenize(s) for s in sentences]:
        ...
        for w in important_words:
            try:
                # compute an index for where any important words in the sentence
...
def summarize(txt):
    ...
    fdist = nltk.FeatDict(words)

    top_n_words  = [w[0] for w in fdist.items() if w[0] not in nltk.corpus.stopwords.word('english')][:N]
